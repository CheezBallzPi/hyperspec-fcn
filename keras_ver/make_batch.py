...
def virt(smp1, smp2):
    ...
    # new = a(old) + bn
    (a, b) = (np.random.random(), np.random.normal(0, 20))
...
def get_samples(X, y, num, classes, window):
    ...
    print(count)
    while(count.min() != num):
        (xpos, ypos) = (np.random.randint(halfwindow - 1, X.shape[0] - halfwindow), 
                        np.random.randint(halfwindow - 1, X.shape[1] - halfwindow))
